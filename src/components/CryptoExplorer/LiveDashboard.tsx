...
export default function LiveDashboard() {
  ...
  const loadOpenPos = useCallback(async () => {
    ...
    const { data } = await supabase
      .from('live_positions')
      .select('id, backtest_run_id, api_key_id, symbol, direction, entry_price, target_price, stop_loss, quantity, capital_used, entry_time, signal_details, score, status, timing_ms, entry_row')
      .eq('status', 'OPEN')
      .eq('user_id', user.id)
  ...
  const loadClosedTrades = useCallback(async () => {
    ...
    const { data } = await supabase
      .from('live_positions')
      .select('id, backtest_run_id, symbol, direction, entry_price, exit_price, net_pnl, pnl_pct, exit_reason, entry_time, exit_time, score, signal_details, exit_details, capital_used, timing_ms, entry_row')
      .eq('status', 'CLOSED')
      .eq('user_id', user.id)
